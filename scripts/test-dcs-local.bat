...
"%CSC%" /nologo /target:exe /platform:x86 /reference:System.ServiceProcess.dll /reference:"%OUT%\DcsData.Historian.dll" /main:DeltaVHistoryCLI.SyncProgram /out:"%OUT%\HistorySync.exe" "%ROOT%\src\HistorySync\HistoryBatch.cs" "%ROOT%\src\HistorySync\SyncState.cs" "%ROOT%\src\HistorySync\HistorySyncService.cs" "%ROOT%\src\HistorySync\HistorySync.cs" "%ROOT%\src\HistorySync\BatchSender.cs" "%ROOT%\src\HistorySync\SpoolMaintenance.cs"
if errorlevel 1 goto failed

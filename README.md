...
生产配置 `DeltaVHistoryCLI_v1.1/config.ini` 和
`HistoryReceiver/receiver.ini` 被 Git 忽略。请从对应的 `*.example.ini` 复制，
不要提交 API Key 或数据库密码。

## Receiver 部署

1. 安装 PostgreSQL，创建数据库和 writer 用户。
2. 执行 `psql -d deltav_history -f HistoryReceiver\sql\create_tables.sql`。
3. 复制 `receiver.example.ini` 为 `receiver.ini`，设置监听地址、API Key 和
...
1. 复制 `config.example.ini` 为 `config.ini`，设置 Receiver 地址和同一 API Key。
2. 在 DCS 电脑执行 `build.bat`。脚本只接受 .NET Framework 2.0/3.5 编译器，
   并生成 x86 EXE。
...
数据库电脑执行 `test-baseline-local.bat`。如果本机没有 .NET 2.0/3.5 编译器，
脚本会明确警告并只做新版框架回归；旧框架兼容性必须在 DCS 电脑运行严格门禁。
